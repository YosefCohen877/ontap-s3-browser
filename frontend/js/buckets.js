...
window.BucketView = (() => {
    ...
    container.innerHTML = buckets.map(b => `
      <div class="bucket-card" role="listitem" tabindex="0"
        ...
        <div class="bucket-card__meta">
          ...
          <span class="bucket-card__count">Counting files…</span>
        </div>
      </div>
    `).join('');
  ...
  function _loadBucketCounts() {
    container.querySelectorAll('.bucket-card').forEach(card => {
      const bucket = card.dataset.bucket;
      const countEl = card.querySelector('.bucket-card__count');
      if (!bucket || !countEl) return;

      API.bucketObjectCount(bucket)
        .then(data => {
          countEl.textContent = `${data.count.toLocaleString()} file${data.count === 1 ? '' : 's'}`;
        })
        .catch(() => {
  ...
  document.getElementById('refreshBucketsBtn')?.addEventListener('click', load);
